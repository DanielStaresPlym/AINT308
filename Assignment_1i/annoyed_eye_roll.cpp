...
void annoyed_eye_roll(int& rx, int& ry, int& lx, int& ly, int& neck)
{
    ...
    if (counter > ACT1_START && counter <= ACT2_START)                                        //Sets Action 1 duration
    {
        ry = static_cast<int>(RyC + (counter / 1000.0) * (RyTm - RyC));                       //Move both eyes in the positive Y direction
        ly = static_cast<int>(LyC - (counter / 1000.0) * (LyC - LyTm));
    }

    if (counter > ACT2_START && counter <= ACT3_START)                                                        //Sets Action 2 duration
    {
        double t = (counter - ACT2_START) / ACT2_DURATION;
        neck = static_cast<int>(NeckC + t * (NeckL  - NeckC) / 4);                                            //Pans the neck slightly
        rx = static_cast<int>(RxC  -  ((RxC - RxLm)) * (sin(2.0 * PI * FREQ * (t)) + 1) / 2.0);               //Move both eyes in clockwise direction for 90 degrees.
        ry = static_cast<int>(RyTm  -  ((RyTm - RyC)) * (-cos(2.0 * PI * FREQ * (t)) + 1) / 2.0);

        lx = static_cast<int>(LxC  -  ((LxRm - LxC)) * (sin(2.0 * PI * FREQ * (t)) + 1) / 2.0);
        ly = static_cast<int>(LyTm  +  ((LyC - LyTm)) * (-cos(2.0 * PI * FREQ * (t)) + 1) / 2.0);
    }

    if (counter > ACT3_START && counter <= ACT4_START)                                      //Sets Action 3 duration
    {
        // delay
    }

    if (counter > ACT4_START && counter <= ACT5_START)                                      //Sets Action 4 duration
    {
    ...
    if (counter > ACT5_START && counter <= ACT_ALL)                                         //Sets Action 5 duration
    {
        // delay
    }
}
